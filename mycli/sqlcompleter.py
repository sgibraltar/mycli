...
class SQLCompleter(Completer):
    refresh_functions = []
    completion_cache_file = "~/.mycli_completion_cache.db"
    ...
    tidb_keywords = [
        ...
        "SECONDARY_UNLOAD",
        "SECOND_MICROSECOND",
        "SECURITY" "SEND_CREDENTIALS_TO_TIKV",
        "SEPARATOR",
        "SEQUENCE",
        "SERIAL",
        "SERIALIZABLE",
    ...
    def extend_columns(self, column_data, kind):
        ...
        for relname, column in column_data:
            metadata[self.dbname].setdefault(relname, []).append(column)
    ...
    def merge_completions(self, other, replace=False):
        if not (other and isinstance(other, dict)):
            return

        if not (self.dbmetadata and isinstance(self.dbmetadata, dict)):
            self.dbmetadata = other
            return

        if self.dbmetadata is other:
            return

        for k, v in other.items():
            if k in self.dbmetadata:
                if replace and v is not None:
                    self.dbmetadata[k] = v
                else:
                    self.dbmetadata[k].update(v)
            else:
                self.dbmetadata[k] = v
    ...
    def get_completions(self, document, complete_event, smart_completion=None):
        ...
        for suggestion in suggestions:
            ...
            if suggestion["type"] == "column":
                ...
                if word_before_cursor and (
                    not suggestion["schema"]
                    and self.databases
                    and len(word_before_cursor.strip()) > 2
                ):
                    matched_dbs = [
                        db
                        for db in self.databases
                        if db.lower().startswith(
                            word_before_cursor.split(".")[0].strip().strip("`").lower()
                        )
                    ]
                    if matched_dbs and (db := matched_dbs[0]) and len(self.populate_schema_objects(db, "tables")) == 0:
                        for refresh_func in self.refresh_functions:
                            if callable(refresh_func):
                                refresh_func(self, scoped_db_name=db, prev_dbmetadata=self.dbmetadata)

                        suggestion["schema"] = db
                        self.set_dbname(db)
